# YUNO AI 2.1 —— 成长型 Agent Memory System

> QQ 里聊天，App 里管理，MCP 串起所有能力。**记忆不是聊天记录，而是 AI 的长期认知。**

YUNO 2.1 是一个把 **AI 长期记忆 / 人格成长 / 关系系统 / 决策辅助 / 生活化世界层** 做成完整认知闭环的 QQ 机器人，
同时把记忆系统抽成可独立接入任意 Agent 的 **Python SDK + FastAPI 服务**。

当前版本：**v2.1**（在 2.0 仓库基础上迭代；功能基线 v31/v32——记忆检索重排 + 多维情绪 +
睡眠/日程/环境/分享/生活/空间/传感器世界层）。

v2.1 相对 2.0 的主要变化：统一记忆系统扩到 7 路融合检索与纠错调查、新增多维情绪/睡眠/
日程/环境/分享/生活/空间/传感器等状态层模块、`persona.md` 人设单一来源 + Hermes 接入、
`docs/` 规划文档迁出仓库、配置与 `.env` 敏感字段清理（凭据需自行填入 `.env`）。
...
- **日程**：`profile=yuno`（夜行、排练/演出/宅家）或 `office`（上班族），种子化周计划——
  同一周稳定、跨周变化，演出前合练/演出后恢复等状态链；用户约定优先于默认日程。
...
| `memory.core.living` | 生活层（家布局、懒散系数、生日） |
| `memory.core.space` | 空间层（场所拓扑、路程时间） |
...
- **测试覆盖薄**：状态层模块（睡眠/日程/生活/空间/分享等）缺单测，无 CI，
  v29 验收脚本还硬编码了本机路径。
- **内存态重启丢失**：`_chat_busy`、检索结果缓存、route_stats 等存在内存里，重启即失。
...
2. **工程化**：状态层补测试 + CI + 内存态落盘 + SQLite 迁移机制 + 锁依赖版本。
3. **产品化**：API Gateway + 管理 Web + 公开统计页，高危操作二次确认，完成"管理迁出 QQ"。
4. **成本与可靠性**：情绪/提取蒸馏成本地轻量模型，回复路径固定为单次 LLM 调用，
   所有 LLM 调用点有降级方案。
5. **认知研究**：空间-时间记忆检索（"X 最近在哪"）、家内空间图、物品事件溯源、
   真实移动、队友确定性日程、遗忘与纠错机制评测。
